...
namespace :fast_ext do
  desc "loading fast-ext 'system' data"
  task :init => :environment do
    system = FastExt::MOrg.create({name: 'system', title: '系统组织', entity_kind:'sys', seq:1, is_active:true})
    administrator = FastExt::MPerson.create({name:'administrator', title:'系统管理员', username:'admin', password:'admin', access_level:'org', entity_kind:'sys', seq:1, is_active:true, createdbyorg:system,})
    attr = {entity_kind:'sys', seq:1, is_active:true, createdbyorg:system, createdby:administrator, updatedby:administrator}

    FastExt::MList.create({name: 'access_level', title: '访问级别'}.merge(attr)) do |al|
      al.m_list_items.build([{name: 'system', title: '系统级别'}, {name: 'org', title: '组织级别'}, {name: 'dept', title: '部门级别'}].map {|a| a.merge(attr)})
    end
    FastExt::MList.create({name: 'entity_kind', title: '实体种类'}.merge(attr)) do |ek|
      ek.m_list_items.build([{name: 'sys', title: '系统实体'}, {name: 'app', title: '应用实体'}].map {|a| a.merge(attr)})
    end
    FastExt::MList.create({name: 'window_kind', title: '窗体种类'}.merge(attr)) do |wk|
      wk.m_list_items.build([{name: 'maintain', title: '维护'}, {name: 'transaction', title: '事务'}, {name: 'query', title: '查询'}].map {|a| a.merge(attr)})
    end

    FastExt::MMenu.create({name: 'sys_menu', title: '系统菜单'}.merge(attr)) do |sm|
